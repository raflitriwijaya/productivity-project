...
- `app.use('/api/todos|finances|learning|research|engineer|links|dashboard|reading|search', generalLimiter, requireAuth, …Router)` (`/api/links` added in Roadmap Wave 1, `/api/dashboard` in Wave 2, `/api/reading` + `/api/search` in Wave 3 — appended last, no reordering; `generalLimiter` is applied globally before the body parsers, so each mount is `requireAuth, …Router`)
