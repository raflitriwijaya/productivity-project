...
### Added — Todo Time Precision + Dual-Channel Reminders
...
- **Telegram reminders:** an in-process scheduler (`server/lib/todoReminder.js`) checks every 5 minutes for todos due within the next 30 minutes and sends an HTML message (priority emoji, title, `HH:MM` due time, description snippet) via a thin `server/lib/telegram.js` wrapper. `reminded_at` keeps a todo from being re-notified within an hour. Reuses the **same** bot token + chat ID as Uptime Kuma / restic backups — one bot, one chat. Wired into the server start/shutdown lifecycle in `index.js`; with `TELEGRAM_*` unset it logs once and no-ops.
- **Browser reminders:** the client poll loop (`lib/notifications.js`) now runs every **5 minutes** (was hourly) and fires an OS notification (`checkTodoReminders`) for todos due within 30 minutes — `requireInteraction` keeps it on screen until dismissed, a per-id `tag` prevents stacking. Reads the shared `/due` feed (one fetch per tick) rather than a second round-trip. Skips silently if notification permission isn't granted. The `/api/notifications/due` todo rows now carry `priority` + `due_time`.
